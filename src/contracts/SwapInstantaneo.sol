...
contract SwapInstantaneo {
    ...
    uint256 public amountGoal; // Monto que una vez superado hace que el swap se ejecute de forma inmediata
    uint256 public currentBalance;
    string public title;
    State public state = State.Fundraising; // Inicializacion del estado del contrato
    mapping(address => uint256) public contributions; // Registro de pagos por cada wallet aportante
    ...
    function swapBNBtoBUSD() public payable inState(State.Successful) {
        ...
        router.swapExactBNBForTokens{value: totalRaised}(
            0,
            path,
            creator,
            block.timestamp
